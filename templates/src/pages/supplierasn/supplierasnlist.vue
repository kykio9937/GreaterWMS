    ...
    <q-card flat bordered class="table-card">
      <div class="table-toolbar">
        <div class="toolbar-tabs">
          <button type="button" class="toolbar-tab toolbar-tab--active">承运商汇总</button>
          <button type="button" class="toolbar-tab" @click="$router.push({ name: 'supplierasnfinish' })">承运商明细</button>
        </div>
      ...
      >
        ...
        <template v-slot:body="props">
          <q-tr :props="props">
            ...
            <q-td key="action" :props="props">
              <q-btn flat dense color="primary" label="[明细]" @click="$router.push({ name: 'supplierasnfinish' })" />
            </q-td>
            <q-td key="organization" :props="props">{{ props.row.organization }}</q-td>
            <q-td key="supplier_name" :props="props">{{ props.row.supplier_name }}</q-td>
            ...
            <q-td key="checked_reward" :props="props" class="number-cell">{{ formatAmount(props.row.checked_reward) }}</q-td>
            <q-td key="unchecked_reward" :props="props" class="number-cell">{{ formatAmount(props.row.unchecked_reward) }}</q-td>
        ...
        <template v-slot:bottom-row>
          <q-tr class="summary-row">
            ...
            <q-td class="number-cell">{{ formatAmount(summary.checked_reward) }}</q-td>
            <q-td class="number-cell">{{ formatAmount(summary.unchecked_reward) }}</q-td>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'supplier_name', label: '承运商名称', field: 'supplier_name', align: 'left' },
        ...
        { name: 'checked_reward', label: '已结司机奖励合计', field: 'checked_reward', align: 'right' },
        { name: 'unchecked_reward', label: '未结司机奖励合计', field: 'unchecked_reward', align: 'right' },
        { name: 'checked_other', label: '已结其他费用合计', field: 'checked_other', align: 'right' },
        { name: 'unchecked_other', label: '未结其他费用合计', field: 'unchecked_other', align: 'right' }
      ]
    }
  },
  computed: {
    ...
    organizationOptions () {
      return [{ label: '全部组织', value: 'all' }].concat(Array.from(new Set(this.sourceRows.map(item => item.organization))).map(item => ({ label: item, value: item })))
    },
    supplierOptions () {
      return [{ label: '全部承运商', value: 'all' }].concat(Array.from(new Set(this.sourceRows.map(item => item.supplier_name))).map(item => ({ label: item, value: item })))
    },
    filteredRows () {
      return this.sourceRows.filter(row => {
        ...
        const matchesSupplier = this.filters.supplierName === 'all' || row.supplier_name === this.filters.supplierName
        ...
        return matchesOrganization && matchesSupplier && matchesColumns
      })
    },
    pagedRows () {
    ...
    summary () {
      return this.filteredRows.reduce((acc, item) => {
        ...
        acc.checked_reward += Number(item.checked_reward || 0)
        acc.unchecked_reward += Number(item.unchecked_reward || 0)
  ...
  methods: {
    ...
    exportData () {
      const header = ['所属组织', '承运商名称', '对账合计', '未对账合计', '总单数', '已结应付运输费合计', '未结应付运输费合计', '已结司机奖励合计', '未结司机奖励合计', '已结其他费用合计', '未结其他费用合计']
      const body = this.filteredRows.map(item => [
        ...
        item.checked_reward,
        item.unchecked_reward,
        item.checked_other,
        item.unchecked_other
      ])
      const content = [header].concat(body).map(line => line.join(',')).join('\r\n')
      exportFile('承运商对账汇总.csv', '\ufeff' + content, 'text/csv')
    },
    async getList () {
      this.loading = true
      try {
        if (this.$q.localStorage.has('auth')) {
          const res = await getauth('dn/reconcile/carrier/summary/', {})
