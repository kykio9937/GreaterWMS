...
def normalize_transportation_fee_payload(dn_obj):
    payload = dn_obj.transportation_fee if isinstance(dn_obj.transportation_fee, dict) else {}
    detail_list = payload.get('detail')
    if not isinstance(detail_list, list):
        detail_list = []
    payload['detail'] = detail_list
    return payload


def extract_finance_meta(dn_obj, dispatch_obj=None):
    payload = normalize_transportation_fee_payload(dn_obj)
    detail_list = payload.get('detail', [])
    first_detail = detail_list[0] if detail_list else {}
    base_income = round(float(first_detail.get('transportation_cost', 0) or 0), 2)
    if base_income <= 0:
        base_income = round(float(dn_obj.total_cost or 0) * 1.15, 2)
    carrier_cost = round(float(payload.get('carrier_cost', 0) or 0), 2)
    if carrier_cost <= 0:
        carrier_cost = round(base_income * 0.92, 2)
    reward_fee = round(float(payload.get('driver_reward', 0) or 0), 2)
    other_fee = round(float(payload.get('extra_fee', 0) or 0), 2)
    line_name = payload.get('line_name') or '{}干线'.format(dn_obj.customer or '默认')
    driver_name = payload.get('driver_name') or ''
    plate_number = payload.get('plate_number') or ''
    supplier_name = payload.get('selected_supplier') or first_detail.get('transportation_supplier') or driver_name or '自有运力'
    if dispatch_obj is not None:
        driver_name = driver_name or dispatch_obj.driver_name
    dispatch_time = payload.get('dispatch_time') or dn_obj.update_time.strftime('%Y-%m-%d %H:%M:%S')
    signed_time = payload.get('signed_time') or ''
    customer_reconciled = bool(payload.get('customer_reconciled', False))
    carrier_reconciled = bool(payload.get('carrier_reconciled', False))
    return {
        'payload': payload,
        'business_income': round(float(payload.get('business_income', 0) or 0), 2) or base_income,
        ...
        'carrier_reconciled': carrier_reconciled
    }


def build_waybill_order_no(dn_code, plate_number):
    tail = str(dn_code or '')[-6:]
    return '{}{}'.format(plate_number or 'KY', tail.rjust(6, '0'))


def build_next_dn_code(openid):
    qs_set = DnListModel.objects.filter(openid=openid)
    order_day = str(timezone.now().strftime('%Y%m%d'))
    if len(qs_set) > 0:
        dn_last_code = qs_set.order_by('-id').first().dn_code
        if str(dn_last_code)[2:10] == order_day:
            order_create_no = str(int(str(dn_last_code)[10:]) + 1)
            return 'DN' + order_day + order_create_no
    return 'DN' + order_day + '1'
...
class DnCreateWaybillViewSet(viewsets.ViewSet):
    def create(self, request):
        ...
        cargo_name = str(data.get('cargo_name') or '').strip()
        ...
        weight = round(float(data.get('weight') or 0), 4)
        volume = round(float(data.get('volume') or 0), 4)
        quantity = int(data.get('quantity') or 1)
        transport_cost = round(float(data.get('transport_cost') or 0), 2)
        arrival_freight = round(float(data.get('arrival_freight') or 0), 2)
        unload_fee = round(float(data.get('unload_fee') or 0), 2)
        storage_fee = round(float(data.get('storage_fee') or 0), 2)
        main_driver = str(data.get('main_driver') or '').strip()
        remark = str(data.get('remark') or '').strip()

        if customer_name == '':
            raise APIException({"detail": "customer_name can not be empty"})
        if cargo_name == '':
            raise APIException({"detail": "cargo_name can not be empty"})
        if quantity < 1:
            quantity = 1

        warehouse_detail = warehouse.objects.filter(openid=openid, is_delete=False).order_by('-id').first()
        if warehouse_detail is None:
            warehouse_detail = warehouse.objects.create(
                openid=openid,
                warehouse_name='中心仓',
                warehouse_city='西安',
                warehouse_address='演示地址',
                warehouse_contact='13800000000',
                warehouse_manager='演示管理员',
                creater='DemoData'
            )

        customer_detail = customer.objects.filter(openid=openid, customer_name=customer_name, is_delete=False).order_by('id').first()
        if customer_detail is None:
            customer_detail = customer.objects.create(
                openid=openid,
                customer_name=customer_name,
                customer_city='西安',
                customer_address=str(data.get('delivery_address') or '演示地址'),
                customer_contact=str(data.get('delivery_phone') or '13800000000'),
                customer_manager=str(data.get('delivery_contact') or customer_name),
                creater=dispatcher or 'DemoData'
            )

        goods_desc = cargo_name
        goods_code = str(data.get('goods_code') or '').strip()
        if goods_code == '':
            goods_code = 'WB' + timezone.now().strftime('%m%d%H%M%S')
        goods_detail = goods.objects.filter(openid=openid, goods_code=goods_code, is_delete=False).order_by('-id').first()
        if goods_detail is None:
            goods_detail = goods.objects.create(
                openid=openid,
                goods_code=goods_code,
                goods_desc=goods_desc,
                goods_supplier='演示供应商',
                goods_weight=max(weight * 1000 / quantity if quantity else 0, 1),
                goods_w=100,
                goods_d=100,
                goods_h=100,
                unit_volume=max(volume / quantity if quantity else 0, 0.001),
                goods_unit='件',
                goods_class='普货',
                goods_brand='演示品牌',
                goods_color='默认',
                ...
                goods_specs='标准',
                goods_origin='中国',
                goods_cost=transport_cost or arrival_freight or 1,
                goods_price=arrival_freight or transport_cost or 1,
                creater=dispatcher or 'DemoData',
                bar_code=Md5.md5(goods_code)
            )

        dn_code = build_next_dn_code(openid)
        transportation_payload = {
            'line_name': line_name or '{}干线'.format(customer_name),
            'plate_number': plate_number,
            'driver_name': main_driver,
            'goods_name': goods_desc,
            'vehicle_type': str(data.get('vehicle_type') or '').strip(),
            'business_income': arrival_freight or transport_cost,
            'carrier_cost': transport_cost,
            'reward_fee': 0,
            'driver_reward': 0,
            'other_fee': unload_fee,
            'extra_fee': unload_fee,
            ...
            'detail': [
                {
                    'transportation_supplier': '演示承运商',
                    'transportation_cost': arrival_freight or transport_cost
                }
            ]
        }
        ...
        dn_obj = DnListModel.objects.create(
            ...
            total_weight=weight,
            total_volume=volume,
            total_cost=arrival_freight or transport_cost,
            customer=customer_name,
            creater=dispatcher or 'DemoData',
            bar_code=Md5.md5(dn_code),
            openid=openid,
            transportation_fee=transportation_payload
        )
        DnDetailModel.objects.create(
            ...
            goods_weight=weight,
            goods_volume=volume,
            goods_cost=arrival_freight or transport_cost,
            creater=dispatcher or 'DemoData',
            openid=openid
        )
        return Response({
            "detail": "success",
            "dn_code": dn_obj.dn_code,
            "id": dn_obj.id
        }, status=200)
        ...
        if qs.dn_status != 4:
            ...
            if driverlist.objects.filter(openid=self.request.auth.openid,
                ...
                driverdispatch.objects.create(openid=self.request.auth.openid,
                                              ...
                                              creater=str(staff_name))
                fee_payload = normalize_transportation_fee_payload(qs)
                detail_list = fee_payload.get('detail', [])
                first_detail = detail_list[0] if detail_list else {}
                carrier_cost = round(float(first_detail.get('transportation_cost', 0) or 0), 2)
                if carrier_cost <= 0:
                    carrier_cost = round(float(qs.total_cost or 0) * 1.05, 2)
                reward_fee = round(carrier_cost * 0.03, 2)
                fee_payload.update({
                    'selected_supplier': first_detail.get('transportation_supplier') or driver.driver_name,
                    'carrier_cost': carrier_cost,
                    'business_income': round(carrier_cost * 1.08, 2),
                    'driver_reward': reward_fee,
                    'extra_fee': round(carrier_cost * 0.01, 2),
                    'driver_name': driver.driver_name,
                    'plate_number': driver.license_plate,
                    'line_name': '{}干线'.format(qs.customer or '默认'),
                    'dispatch_time': timezone.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'customer_reconciled': bool(fee_payload.get('customer_reconciled', False)),
                    'carrier_reconciled': bool(fee_payload.get('carrier_reconciled', False))
                })
                qs.transportation_fee = fee_payload
                qs.save()
                return Response({"detail": "success"}, status=200)
            else:
        ...
        if qs.dn_status != 5:
            ...
            for j in range(len(data['goodsData'])):
                ...
                if delivery_damage_qty > 0:
                    ...
                    goods_detail.save()
            fee_payload = normalize_transportation_fee_payload(qs)
            fee_payload['signed_time'] = timezone.now().strftime('%Y-%m-%d %H:%M:%S')
            qs.transportation_fee = fee_payload
            qs.save()
            return Response({"detail": "success"}, status=200)


class DnTransitViewSet(viewsets.ViewSet):
    """
        list:
            返回在途与已签收运单的跟踪与费用数据
    """
    ...
    def list(self, request):
        if not request.user:
            return Response([])
        dn_list = DnListModel.objects.filter(
            openid=request.auth.openid,
            dn_status__gte=5,
            is_delete=False
        ).order_by('-update_time')
        dispatch_map = {}
        for item in driverdispatch.objects.filter(openid=request.auth.openid).order_by('-create_time'):
            if item.dn_code not in dispatch_map:
                dispatch_map[item.dn_code] = item
        rows = []
        for index, dn_obj in enumerate(dn_list):
            finance_meta = extract_finance_meta(dn_obj, dispatch_map.get(dn_obj.dn_code))
            progress = 100 if dn_obj.dn_status == 6 else min(95, 52 + (index % 5) * 9)
            rows.append({
                'id': dn_obj.id,
                'dn_code': dn_obj.dn_code,
                'order_no': build_waybill_order_no(dn_obj.dn_code, finance_meta['plate_number']),
                'customer': dn_obj.customer,
                'line_name': finance_meta['line_name'],
                'dn_status': dn_obj.dn_status,
                'waybill_status': '已签收' if dn_obj.dn_status == 6 else '运输中',
                'driver_name': finance_meta['driver_name'],
                'plate_number': finance_meta['plate_number'],
                'supplier_name': finance_meta['supplier_name'],
                'location_track': '{}运输节点'.format(dn_obj.customer or '在途'),
                'monitor_alarm': '正常' if dn_obj.dn_status == 6 else '定位跟踪',
                'mileage_progress': progress,
                'temperature': '--',
                'humidity': '--',
                'business_income': finance_meta['business_income'],
                'carrier_cost': finance_meta['carrier_cost'],
                'reward_fee': finance_meta['reward_fee'],
                'other_fee': finance_meta['other_fee'],
                'customer_reconcile_status': '已对账' if finance_meta['customer_reconciled'] else '待对账',
                'carrier_reconcile_status': '已对账' if finance_meta['carrier_reconciled'] else '待对账',
                'dispatch_time': finance_meta['dispatch_time'],
                'signed_time': finance_meta['signed_time'],
                'create_time': dn_obj.create_time.strftime('%Y-%m-%d %H:%M:%S')
            })
        return Response(rows, status=200)


class DnCustomerReconcileSummaryViewSet(viewsets.ViewSet):
    def list(self, request):
        if not request.user:
            return Response([])
        dn_list = DnListModel.objects.filter(openid=request.auth.openid, dn_status=6, is_delete=False).order_by('-update_time')
        summary_map = {}
        for index, dn_obj in enumerate(dn_list):
            finance_meta = extract_finance_meta(dn_obj)
            customer_name = dn_obj.customer or '未命名客户'
            ...
            item = summary_map[group_key]
            item['total_count'] += 1
            if finance_meta['customer_reconciled']:
                item['checked_total'] += finance_meta['business_income']
                item['checked_receivable'] += finance_meta['business_income']
                item['checked_payable'] += finance_meta['carrier_cost']
                item['checked_other'] += finance_meta['other_fee']
            else:
                item['unchecked_total'] += finance_meta['business_income']
                item['unchecked_receivable'] += finance_meta['business_income']
                item['unchecked_payable'] += finance_meta['carrier_cost']
                item['unchecked_other'] += finance_meta['other_fee']
        return Response(list(summary_map.values()), status=200)


class DnCustomerReconcileDetailViewSet(viewsets.ViewSet):
    def list(self, request):
        if not request.user:
            return Response([])
        dispatch_map = {}
        ...
        for dn_obj in dn_list:
            ...
            rows.append({
                ...
                'plate_number': finance_meta['plate_number'],
                'line_name': finance_meta['line_name'],
                'receivable_fee': finance_meta['business_income'],
                'payable_fee': finance_meta['carrier_cost'],
                'settle_status': '已对账' if finance_meta['customer_reconciled'] else '待对账',
                'signed_time': finance_meta['signed_time']
            })
        return Response(rows, status=200)


class DnCustomerReconcileConfirmViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, pk):
        ...
        if qs.openid != request.auth.openid:
            raise APIException({"detail": "Cannot update data which not yours"})
        fee_payload = normalize_transportation_fee_payload(qs)
        fee_payload['customer_reconciled'] = True
        fee_payload['customer_reconcile_time'] = timezone.now().strftime('%Y-%m-%d %H:%M:%S')
        qs.transportation_fee = fee_payload
        qs.save(update_fields=['transportation_fee', 'update_time'])
        return Response({"detail": "success"}, status=200)


class DnCarrierReconcileSummaryViewSet(viewsets.ViewSet):
    def list(self, request):
        if not request.user:
            return Response([])
        dispatch_map = {}
        ...
        for index, dn_obj in enumerate(dn_list):
            finance_meta = extract_finance_meta(dn_obj, dispatch_map.get(dn_obj.dn_code))
            supplier_name = finance_meta['supplier_name'] or '自有运力'
            if supplier_name not in summary_map:
                summary_map[supplier_name] = {
                    'id': index + 1,
                    'organization': '运营中心',
                    'supplier_name': supplier_name,
                    'checked_total': 0,
            ...
            item = summary_map[supplier_name]
            item['total_count'] += 1
            group_total = finance_meta['carrier_cost'] + finance_meta['reward_fee'] + finance_meta['other_fee']
            if finance_meta['carrier_reconciled']:
                item['checked_total'] += group_total
                item['checked_payable'] += finance_meta['carrier_cost']
                item['checked_reward'] += finance_meta['reward_fee']
                item['checked_other'] += finance_meta['other_fee']
            else:
                item['unchecked_total'] += group_total
                item['unchecked_payable'] += finance_meta['carrier_cost']
                item['unchecked_reward'] += finance_meta['reward_fee']
                item['unchecked_other'] += finance_meta['other_fee']
        return Response(list(summary_map.values()), status=200)


class DnCarrierReconcileDetailViewSet(viewsets.ViewSet):
    def list(self, request):
        if not request.user:
            return Response([])
        dispatch_map = {}
        ...
        for dn_obj in dn_list:
            ...
            rows.append({
                ...
                'dn_code': dn_obj.dn_code,
                'order_no': build_waybill_order_no(dn_obj.dn_code, finance_meta['plate_number']),
                'supplier': finance_meta['supplier_name'],
                'vehicle': finance_meta['plate_number'],
                'payable_fee': finance_meta['carrier_cost'],
                'reward_fee': finance_meta['reward_fee'],
                'other_fee': finance_meta['other_fee'],
                'status': '已对账' if finance_meta['carrier_reconciled'] else '待对账'
            })
        return Response(rows, status=200)


class DnCarrierReconcileConfirmViewSet(viewsets.ModelViewSet):
    queryset = DnListModel.objects.all()
    ...
    def create(self, request, pk):
        ...
        if qs.openid != request.auth.openid:
            raise APIException({"detail": "Cannot update data which not yours"})
        fee_payload = normalize_transportation_fee_payload(qs)
        fee_payload['carrier_reconciled'] = True
        fee_payload['carrier_reconcile_time'] = timezone.now().strftime('%Y-%m-%d %H:%M:%S')
        qs.transportation_fee = fee_payload
        qs.save(update_fields=['transportation_fee', 'update_time'])
        return Response({"detail": "success"}, status=200)
